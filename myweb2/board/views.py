...
def list(request):
    ...
    page_list_size=10 # 한 화면에 표시할 페이지의 개수
    end=start+page_size
    ...
    if search_option == 'all':
        boardList=Board.objects.filter(\
            Q(writer__contains=search)|Q(title__contains=search)|Q(content__contains=search)).order_by("-idx")[start:end]
    ...
    return render(request, "list.html",\
                  {"boardList": boardList, "boardCount": boardCount, "search_option": search_option,
                   "search": search, "range": range(start_page-1,end_page), "start_page": start_page,
                   "end_page": end_page, "page_list_size": page_list_size, "total_page": total_page,
...
def update(request):
    ...
    row_new=Board(idx=id, writer=request.POST["writer"], title=request.POST["title"],
              content=request.POST["content"], filename=fname, filesize=fsize, hit=hit, down=down)
    row_new.save()
    return redirect("/")
